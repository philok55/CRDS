...
int MYMPI_Bcast(MPI_Datatype datatype, int root, MPI_Comm communicator, int count, void *buffer)
{
...
void swap_buffers(double **cur, double **old, double **new)
{
    double *temp = *old;
    *old = *cur;
    *cur = *new;
...
double *simulate(double *old_array, double *next_array, double *current_array, const int i_max, const int t_max)
{
    ...
    if (task_id == 0)
    {
        ...
        for (int j = 0; j < (rest + local_size); j++)
        {
    ...
    for (int t = 0; t < t_max; t++)
    {
        ...
        if (task_id == 0)
        {
            for (int i = 2; i <= rest + local_size; i++)
            {
    ...
    if (task_id == 0)
    {
        for (int i = 1; i <= rest + local_size; i++)
