...
def task1a(G, source, dest):
    ...
    if dest is None:  # No destination: return information about all nodes
        return prev, dist
...
def find_shortest_paths(G, lengths):
    ...
    for node1 in G.nodes:
        for node2 in G.nodes:
            if ((node1, node2) in traversed or
               (node2, node1) in traversed or node1 == node2):
                continue
...
def task4a(G):
    ...
    for node1 in G.nodes:
        for node2 in G.nodes:
            if (node1 == node2 or
               (node1, node2) in checked or
               (node2, node1) in checked or
