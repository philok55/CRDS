...
class Sensor():
    ...
    def calc_distance(self, pos1, pos2=None):
        ...
        """
        if pos2 is None:
            return math.sqrt(abs(self.pos[0] - pos1[0]) ** 2 +
                             abs(self.pos[1] - pos1[1]) ** 2)
        return math.sqrt(abs(pos1[0] - pos2[0]) ** 2 +
                         abs(pos1[1] - pos2[1]) ** 2)
    ...
    def calc_range(self, pos1, pos2=None):
        ...
        if pos2 is None:
            if distance == 0.0:  # Message from yourself
                return False
    ...
    def process_message(self, data, address):
        ...
        appropriate methods.
        """
        type, seq_num, init, neighbor, operation, _, _, payload = \
            message_decode(data)
        if type == MSG_PONG:
    ...
    def handle_echo_reply(self, seq_num, init, neighbor, operation, payload):
        ...
        if self.pos == init:
            if (len(self.echo_replies[key]) == self.echos_sent[key]):
                ...
                if operation == OP_SIZE:
                    msg = ("Size of Network: " +
                           str(int(self.neighbors_sums[key] + 1)) +
                           " sensors.")
        ...
        if (len(self.echo_replies[key]) == self.echos_sent[key]):
            ...
            self.echo_reply(self.fathers[key], seq_num, init,
                            operation, payload)
    ...
    def echo(self, init, seq_num, operation):
        ...
        key = str((init, seq_num))
        msg = message_encode(MSG_ECHO, seq_num, init, self.pos, operation,
                             strength=self.strength,
                             decay=self.decay)
        self.seen_echos.append((init, seq_num))
        if operation == OP_SIZE:
    ...
    def echo_reply(self, dest, seq_num, init, operation, payload):
        ...
        msg = message_encode(MSG_ECHO_REPLY, seq_num, init, self.pos,
                             operation,
                             strength=self.strength,
                             decay=self.decay,
                             payload=payload)
    ...
    def calc_route(self, dest, typ):
        ...
        if total_distance == float("inf"):
            self.window.writeln("There is no path between " + str(self.pos) +
                                " and " + str(dest) + " in the sensor network")
            return
        if typ == ROUTE_DECAY:
            t = "decay"
        else:
            t = "strength"
        self.window.writeln("The shortest path to " + str(dest) + " is: " +
                            str(path))
    ...
    def find_all_paths(self):
        ...
        for sensor1 in self.known_sensors:
            for sensor2 in self.known_sensors:
                if ((sensor1, sensor2) in traversed or
                        (sensor2, sensor1) in traversed or sensor1 == sensor2):
                    continue
...
def main(mcast_addr, sensor_pos, sensor_strength, sensor_decay,
    ...
    sensor = Sensor(mcast_addr, sensor_pos, sensor_strength, sensor_decay,
                    window, grid_size)
    ...
    while window.update():
        readable, writable, exc = select.select(read_list, write_list,
                                                write_list, 0)
        ...
        for sock in readable:
            if sock == mcast:
                ...
                if data:
                    _, _, pos_init, _, _, str_init, decay_init, _ = \
                        message_decode(data)
