...
def main():
    ...
    for i in range(len(times)):
        ...
        if i == 0:
            print(">> Transition at " + str(times[i]) +
                  " to slow start. Value of cwnd: " +
                  str(cwnd[i]) + ".")
            continue
        ...
        if cwnd[i] < cwnd[i - 1]:
            ...
            if cwnd[i] == MSS:
                colors[i - 1] = [1, 1, 0, 1]
                print(">> Timeout at " + str(times[i - 1]) +
                      ". Value of cwnd: " + str(cwnd[i - 1]) + ".")
                if not slow_start:
                    print(">> Transition at " + str(times[i - 1]) +
                          " to slow start. Value of cwnd: " +
                          str(cwnd[i - 1]) + ".")
                avoidance = False
                fast_recovery = False
                ...
                if not fast_recovery:
                    print(">> Transition at " + str(times[i - 1]) +
                          " to fast recovery. Value of cwnd: " +
                          str(cwnd[i - 1]) + ".")
                    fast_recovery = True
                    slow_start = False
                    avoidance = False
            ...
            if fast_recovery or slow_start:
                if cwnd[i] == cwnd[i - 1] + MSS * math.ceil(MSS/cwnd[i - 1]):
                    print(">> Transition at " + str(times[i - 1]) +
                          " to congestion avoidance. Value of cwnd: " +
                          str(cwnd[i - 1]) + ".")
                    avoidance = True
                    fast_recovery = False
                    slow_start = False
    ...
    ax.autoscale()
    ax.margins(0.1)
    avoidance_patch = mpatches.Patch(color=[1, 0, 0, 1],
                                     label='congestion avoidance')
    slow_start_patch = mpatches.Patch(color=[0, 0, 1, 1], label='slow start')
    fast_recovery_patch = mpatches.Patch(color=[0, 1, 0, 1],
                                         label='fast recovery')
    timeout_patch = mpatches.Patch(color=[1, 1, 0, 1], label='timeout')
    plt.legend(handles=[avoidance_patch, slow_start_patch, fast_recovery_patch,
                        timeout_patch])
