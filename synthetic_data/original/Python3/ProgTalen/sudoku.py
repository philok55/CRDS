...
#
# sudoku.py:
# Dit programma lost een sudoku op van willekeurige grootte. Een grid dat moet
# worden opgelost wordt als argument meegegeven aan het programma, en de
# opgeloste sudoku wordt geprint. De oplossing wordt gevonden zonder recursie.
...
def read_blocks(rows, blocksize):
    ...
    terug.
    """
    return [[rows[blocksize*i1 + i3][blocksize*i2 + i4]
            for i3 in range(blocksize)
            for i4 in range(blocksize)]
            for i1 in range(blocksize)
            for i2 in range(blocksize)]
    ...
    for row_index, row in enumerate(rows):
        for number_index, number in enumerate(row):
            if number == 0:
                ...
                possibilities = find_numbers(rows[row_index], columns[number_index],
                                        blocks[block_index], blocksize)
...
# Als er geen bestand is meegegeven aan het programma wordt een foutmelding
# gegeven.
if not len(sys.argv) >= 2:
    sys.exit("Usage: python sudoku.py [input file]")

# Hier wordt de main functie aangeroepen.
