intMYMPI_Bcast(void *buffer, intcount, MPI_Datatypedatatype, introot, MPI_Commcommunicator)
{
...
voidswap_buffers(double **old, double **cur, double **new)
{
    double *temp = *old;
    *old = *cur;
    *cur = *new;
...
double *simulate(constinti_max, constintt_max, double *old_array, double *current_array, double *next_array)
{
    ...
    if (task_id == 0)
    {
        ...
        for (intj = 0; j < (local_size + rest); j++)
        {
    ...
    for (intt = 0; t < t_max; t++)
    {
        ...
        if (task_id == 0)
        {
            for (inti = 2; i <= local_size + rest; i++)
            {
    ...
    if (task_id == 0)
    {
        for (inti = 1; i <= local_size + rest; i++)
